...
namespace Core.Devices
{
    public class Diode : Node
    {
        ...
        protected override void Awake()
        {
            ...
            lightMaterial = lightRenderer.materials[3];
        }
        ...
        protected override void Update()
        {
            ...
            float current = Mathf.Abs(GetCurrent());
            if (current <= 0.01f) lightMaterial.DisableKeyword("_EMISSION");
            else lightMaterial.EnableKeyword("_EMISSION");
            Color color = Color.Lerp(Color.black, lightColor, 100f * current);
            lightMaterial.SetColor("_EmissionColor", color);
            light.color = color;
            light.intensity = Mathf.Sqrt(100f * current);
        }
